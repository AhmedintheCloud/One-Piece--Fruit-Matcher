...
## The Game
A simple memory matching game based on the renowned One Piece Franchise.The user clicks two cards (images of characters and their corresponding devil fruits). If there's a match, the cards disappear from the board.  If there's no match, the cards are flipped back to their blank side so the user can try again.
